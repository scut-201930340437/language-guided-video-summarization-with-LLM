...
def segmentation(frames_features, m):
    ### Vlog kts ###
    # frames_features = frames_features / (np.linalg.norm(frames_features, axis=-1, keepdims=True) + 1e-5)
    
    ### our kts ###
    frames_features = np.array(frames_features)
    for i in range(len(frames_features)):
        frames_features[i] = frames_features[i] / np.linalg.norm(frames_features[i])

    n = frames_features.shape[0]
    ...
    K = np.dot(frames_features, frames_features.T)
    cps, scores = cpd_auto(K, m, vmax=1)
