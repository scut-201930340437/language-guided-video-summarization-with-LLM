...
from utils.read_utils import read_TVSum_anno, read_summary
...
def cal_spearAndken_dif_len(pred_frames_score, pred_indicator, user_score, userIdx, segments_idx):
    nFrames = len(pred_frames_score)
    gt_score = np.fromiter(map(lambda x: x, user_score[:, userIdx]), dtype=int)
    gt_score = gt_score[:nFrames]
...
def cal_spearAndken_same_len(pred_frames_score, pred_indicator, user_score, userIdx):
    ...
    gt_indicator = gt_indicator[:nFrames]
    ...
    if len(gt_score) < max_len:
        gt_score = np.concatenate((gt_score, np.zeros(max_len - len(gt_score), dtype=int)), axis=0)
    elif len(pred_frames_score_copy) < max_len:
        pred_frames_score_copy = np.concatenate((pred_frames_score_copy, np.zeros(max_len - len(pred_frames_score_copy), dtype=int)), axis=0)
...
def evaluate_SumMe(pred_frames_score, video_path, video_name, clip_model, clip_preprocess, device, segments_idx):
    ...
    user_length = np.zeros(nbOfUsers)
    pred_length = np.zeros(nbOfUsers)
    # spearman
    ...
    for userIdx in range(nbOfUsers):
        ...
        # segments_idx = [0]
        # for gt_indicator_idx in range(1, len(gt_indicator)):
        #     if gt_indicator[gt_indicator_idx] != gt_indicator[gt_indicator_idx - 1]:
        #         segments_idx.append(gt_indicator_idx)
        # segments_idx.append(len(gt_indicator))
        ...
        user_spearman_dif_len[userIdx], user_kendall_dif_len[userIdx] = cal_spearAndken_dif_len(pred_frames_score, pred_indicator, user_score, userIdx, segments_idx)
        ...
        user_length[userIdx] = gt_length
        pred_length[userIdx] = np.sum(pred_indicator)

    recall = user_intersection / user_length
    p = user_intersection / pred_length
...
def evaluate_TVSum(pred_frames_score, video_path, video_name, clip_model, clip_preprocess, device, gt_sumy_rate, segments_idx):
    ...
    pred_length = np.zeros(nbOfUsers, dtype=int)
    # spearman
    user_spearman_dif_len = np.zeros(nbOfUsers)
    user_spearman_same_len = np.zeros(0)
    # kendall
    ...
    gt_indicators = read_summary('TVSum', video_name)
    ...
    for userIdx in range(nbOfUsers):
        ...
        # gt_indicator = gt_indicators[userIdx]
